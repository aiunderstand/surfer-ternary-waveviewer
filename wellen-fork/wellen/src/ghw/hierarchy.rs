...
impl GhwSignalTracker {
    ...
    fn into_decode_info(self) -> (GhwDecodeInfo, Vec<AliasInfo>) {
        ...
        ((GhwSignals::new(signals), self.vectors, self.enum_vecs), self.aliases)
    }
    ...
    fn register_enum_vec(
        &mut self,
        signal_ids: Vec<GhwSignalId>,
        bits_per_element: u8,
    ) -> SignalRef {
        let min = *signal_ids.first().unwrap();
        let max = *signal_ids.last().unwrap();
        let id = self.new_signal_ref();
        let enum_vec_id = self.enum_vecs.len();

        self.enum_vecs.push(EnumVecInfo::new(min, max, bits_per_element, id));

        // Register each element signal slot pointing to this enum vec
        for signal_id in signal_ids.iter() {
            debug_assert!(
                self.signals[signal_id.index()].is_none(),
                "signal slot already occupied"
            );
            self.signals[signal_id.index()] = Some(GhwSignalInfo::new(
                SignalType::EnumVec,
                id,
                Some(enum_vec_id),
            ));
...
fn add_var(
    ...
    match vhdl_tpe {
        ...
        VhdlType::Array(_, element_tpe, maybe_range) => {
            ...
            // Check if element type is Enum — if so, store as a single combined signal
            if let VhdlType::Enum(_, literals, enum_id) = &tables.types[element_tpe.index()] {
                let bits_per_element = get_enum_bits(literals) as u8;
                let num_elements = range.len().unsigned_abs() as usize;
                let total_bits = bits_per_element as u32 * num_elements as u32;

                // Read one signal ID per element
                let mut signal_ids = Vec::with_capacity(num_elements);
                for _ in 0..num_elements {
                    signal_ids.push(read_signal_id(input, signals.max_signal_id())?);
                }

                let enum_type = tables.enums[*enum_id as usize];
                let signal_ref = signals.register_enum_vec(signal_ids, bits_per_element);

                h.add_var(
                    name,
                    VarType::EnumArray,
                    crate::hierarchy::SignalEncoding::bit_vec_of_len(total_bits),
                    dir,
                    Some(range.as_var_index()),
                    signal_ref,
                    Some(enum_type),
                ...
                // Non-enum array — existing behavior: treat like a record/scope
