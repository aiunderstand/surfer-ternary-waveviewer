...
impl GhwSignalInfo {
    pub fn new(tpe: SignalType, signal_ref: SignalRef, vector: Option<usize>) -> Self {
        ...
        debug_assert_eq!(raw_tpe & 0xF, raw_tpe);
        let tpe_and_vec = if let Some(vector) = vector {
            let raw_vector_id = (vector as u32) + 1;
            debug_assert_eq!((raw_vector_id << 4) >> 4, raw_vector_id);
            NonZeroU32::new((raw_vector_id << 4) | raw_tpe).unwrap()
        } else {
    ...
    pub fn vec_id(&self) -> Option<GhwVecId> {
        let vec_id = self.tpe_and_vec.get() >> 4;
        if vec_id == 0 {
            None
        } else {
    ...
    pub fn tpe(&self) -> SignalType {
        ...
        let raw_tpe = (value & 0xF) as u8;

        SignalType::try_from_primitive(raw_tpe - 1).unwrap()
    }
}

pub type GhwDecodeInfo = (GhwSignals, Vec<GhwVecInfo>, Vec<EnumVecInfo>);
...
/// Tracks a vector of enum elements, where each element is stored as a fixed-width ordinal.
/// Unlike `GhwVecInfo` which tracks bit-level signals, this tracks element-level signals.
#[derive(Debug, Clone, Copy)]
pub struct EnumVecInfo {
    /// GhwSignalId of the first (highest-indexed in downto) element
    pub min: GhwSignalId,
    /// GhwSignalId of the last element
    pub max: GhwSignalId,
    /// Number of bits needed to encode one enum ordinal
    pub bits_per_element: u8,
    /// The combined signal reference in the wellen output
...
impl EnumVecInfo {
    pub fn new(
        ...
        Self { min, max, bits_per_element, signal_ref }
    }
    ...
    pub fn total_bits(&self) -> u32 {
        self.num_elements() * self.bits_per_element as u32
    }
}

/// Pointer to an `EnumVecInfo`
#[derive(Debug, Clone, Copy, PartialEq)]
pub struct EnumVecId(NonZeroU32);
...
pub enum SignalType {
    ...
    /// F64 (real)
    F64,
    /// A single element slot in an enum array vector. Each slot holds bits_per_element bits.
    EnumVec,
}
