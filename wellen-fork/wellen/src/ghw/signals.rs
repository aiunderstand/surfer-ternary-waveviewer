...
pub fn read_signals(
    ...
    let (info, vectors, enum_vecs) = decode_info;
    // TODO: multi-threading
    let mut encoder = Encoder::new(hierarchy);
    let mut vecs = VecBuffer::from_vec_info(vectors);
    let mut enum_vec_buf = EnumVecBuffer::new(enum_vecs);
    ...
    loop {
        ...
        match &mark {
            GHW_SNAPSHOT_SECTION => {
                read_snapshot_section(header, &info, &mut vecs, &mut enum_vec_buf, &mut encoder, input)?
            }
            GHW_CYCLE_SECTION => read_cycle_section(header, &info, &mut vecs, &mut enum_vec_buf, &mut encoder, input)?,
...
fn read_snapshot_section(
    ...
    info: &GhwSignals,
    vecs: &mut VecBuffer,
    enum_vec_buf: &mut EnumVecBuffer,
    enc: &mut Encoder,
    input: &mut impl BufRead,
) -> Result<()> {
    let mut h = [0u8; 12];
    ...
    for sig_index in 0..(info.signal_len() as u32) {
        read_signal_value(info, GhwSignalId::new(sig_index + 1), vecs, enum_vec_buf, enc, input)?;
    }
    finish_time_step(vecs, enum_vec_buf, enc);
...
fn read_cycle_section(
    ...
    loop {
        enc.time_change(start_time);
        read_cycle_signals(info, vecs, enum_vec_buf, enc, input)?;
        finish_time_step(vecs, enum_vec_buf, enc);
...
fn read_cycle_signals(
    ...
    loop {
        ...
        read_signal_value(info, sig_id, vecs, enum_vec_buf, enc, input)?;
    }
    Ok(())
}

/// This dispatches any remaining vector changes.
fn finish_time_step(vecs: &mut VecBuffer, enum_vec_buf: &mut EnumVecBuffer, enc: &mut Encoder) {
    vecs.process_changed_signals(|signal_ref, data, states| {
        enc.raw_value_change(signal_ref, data, states);
    });
    enum_vec_buf.process_changed_signals(|signal_ref, data| {
        enc.raw_value_change(signal_ref, data, States::Two);
    });
}
...
fn read_signal_value(
    ...
    match tpe {
        ...
        SignalType::EnumVec => {
            let ordinal = read_u8(input)?;
            let enum_vec_id = EnumVecId::new(signal_info.vec_id().unwrap().index() + 1);
            enum_vec_buf.update_element(enum_vec_id, signal_id, ordinal);
            // Emit immediately if all elements have been updated this timestep
            if enum_vec_buf.all_elements_changed(enum_vec_id) {
                let info = &enum_vec_buf.info[enum_vec_id.index()];
                let signal_ref = info.signal_ref;
                let data = enum_vec_buf.get_data_and_clear(enum_vec_id);
                enc.raw_value_change(signal_ref, data, States::Two);
            }
        }
    }
...
/// Tracks enum array signals. Each element is a fixed-width ordinal packed
/// into a combined binary bit string — e.g. 11 elements × 4 bits = 44 bits.
struct EnumVecBuffer {
    info: Vec<EnumVecInfo>,
    /// Raw packed data: elements stored MSB-first, each element occupying
    /// bits_per_element bits, packed tightly into bytes.
    data: Vec<u8>,
    /// One bit per element indicating whether it has been updated this timestep.
    element_changed: Vec<u8>,
    /// One bit per vector indicating whether any element changed.
    signal_change: Vec<u8>,
    change_list: Vec<EnumVecId>,
}

impl EnumVecBuffer {
    fn new(enum_vecs: Vec<EnumVecInfo>) -> Self {
        let mut data_len = 0usize;
        let mut changed_len = 0usize;

        for v in &enum_vecs {
            let total_bits = v.total_bits() as usize;
            data_len += total_bits.div_ceil(8);
            changed_len += (v.num_elements() as usize).div_ceil(8);
        }

        Self {
            data: vec![0u8; data_len],
            element_changed: vec![0u8; changed_len],
            signal_change: vec![0u8; enum_vecs.len().div_ceil(8)],
            change_list: vec![],
            info: enum_vecs,
        }
    }
    ...
    fn update_element(
        &mut self,
        enum_vec_id: EnumVecId,
        signal_id: GhwSignalId,
        ordinal: u8,
    ) {
        let info = &self.info[enum_vec_id.index()];
        let element_idx = signal_id.index() - info.min.index();
        let bpe = info.bits_per_element as usize;
        let total_bits = info.total_bits() as usize;
        let data_start = self.data_start(enum_vec_id);
        let data_len = total_bits.div_ceil(8);

        // Pack right-aligned: element 0 (first read = MSB in output) occupies
        // bit positions [total_bits - bpe, total_bits) from LSB of the byte array.
        // This ensures BigUint::from_bytes_be(data) == the N-bit packed integer.
        let lsb_of_element = total_bits - (element_idx + 1) * bpe;

        for b in 0..bpe {
            let bit_val = (ordinal >> b) & 1; // b=0 is LSB of ordinal
            let bit_pos_from_lsb = lsb_of_element + b;
            let byte_from_end = bit_pos_from_lsb / 8;
            let bit_in_byte = bit_pos_from_lsb % 8;
            let byte_idx = data_start + data_len - 1 - byte_from_end;
        ...
        // Mark element changed
        let changed_start = self.changed_start(enum_vec_id);
        let byte = changed_start + element_idx / 8;
        self.element_changed[byte] |= 1 << (element_idx % 8);

        if !self.has_signal_changed(enum_vec_id) {
            self.mark_signal_changed(enum_vec_id);
        }
    }

    fn has_signal_changed(&self, id: EnumVecId) -> bool {
        let byte = id.index() / 8;
        let bit = id.index() % 8;
        (self.signal_change[byte] >> bit) & 1 == 1
    }

    fn mark_signal_changed(&mut self, id: EnumVecId) {
        let byte = id.index() / 8;
        let bit = id.index() % 8;
        self.signal_change[byte] |= 1 << bit;
        self.change_list.push(id);
    }

    fn all_elements_changed(&self, id: EnumVecId) -> bool {
        let info = &self.info[id.index()];
        let n = info.num_elements() as usize;
        let start = self.changed_start(id);
        let full_bytes = n / 8;
        for i in 0..full_bytes {
            if self.element_changed[start + i] != 0xff {
                return false;
            }
        }
        let remainder = n % 8;
        if remainder > 0 {
            ...
            if self.element_changed[start + full_bytes] & mask != mask {
                return false;
            }
        }
        true
    ...
    fn get_data_and_clear(&mut self, id: EnumVecId) -> &[u8] {
        let info = &self.info[id.index()];
        let n = info.num_elements() as usize;

        // Clear element_changed
        let changed_start = self.changed_start(id);
        let bytes = (n).div_ceil(8);
        for i in 0..bytes {
            self.element_changed[changed_start + i] = 0;
        }
        ...
        let start = self.data_start(id);
        let len = (info.total_bits() as usize).div_ceil(8);
    ...
    fn process_changed_signals(&mut self, mut callback: impl FnMut(SignalRef, &[u8])) {
        ...
        for id in change_list {
            if self.has_signal_changed(id) {
                let signal_ref = self.info[id.index()].signal_ref;
                let data = self.get_data_and_clear(id);
                (callback)(signal_ref, data);
            }
        }
    }

    fn data_start(&self, id: EnumVecId) -> usize {
        self.info[..id.index()]
            .iter()
            .map(|v| (v.total_bits() as usize).div_ceil(8))
            .sum()
    }

    fn changed_start(&self, id: EnumVecId) -> usize {
        self.info[..id.index()]
            .iter()
            .map(|v| (v.num_elements() as usize).div_ceil(8))
            .sum()
    }
}
