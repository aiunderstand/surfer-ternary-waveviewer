...
use surfer_translation_types::{
    TranslationPreference, TranslationResult, Translator, TriRail, ValueKind, ValueRepr,
...
pub struct BalancedTernaryTranslator;

impl Translator<VarId, ScopeId, Message> for BalancedTernaryTranslator {
    fn name(&self) -> String {
        "Balanced Ternary".to_string()
    }
    ...
    fn translate(&self, variable: &VariableMeta, value: &VariableValue) -> Result<TranslationResult> {
        let result = match variable.variable_type_name.as_deref() {
            Some("btern_ulogic_vector") | Some("btern_logic_vector") => {
                ...
                translate_btern_vector(value.clone(), num_bits)
            }
            Some("kleene_vector") => {
                let num_bits = variable.num_bits.unwrap_or(0);
                translate_kleene_vector(value.clone(), num_bits)
            }
            Some("kleene") => translate_kleene(value.clone()),
            Some("btern_ulogic") | Some("btern_logic") => translate_btern(value.clone()),
